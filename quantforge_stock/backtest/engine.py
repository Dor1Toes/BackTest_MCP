...
class _SymbolBuffer:
    """Append-only numeric buffer for per-symbol only OHLCV + timestamps."""

    __slots__ = ("_ts", "_cols", "_n", "_cap")

    def __init__(self, initial_cap: int = 256) -> None:
        self._ts: list[pd.Timestamp] = []
        self._cols: dict[str, np.ndarray] = {col: np.empty(initial_cap, dtype=np.float64) for col in _OHLCV_COLS}
        self._n = 0
        self._cap = initial_cap

    def _grow(self, new_cap: int) -> None:
        for col in _OHLCV_COLS:
            buffer = np.empty(new_cap, dtype=np.float64)
            buffer[:self._n] = self._cols[col][:self._n]
            self._cols[col] = buffer
        self._cap = new_cap
    ...
    def as_frame(self, tail:int | None = None) -> pd.DataFrame:
        ...
        return pd.DataFrame(
                {col : self._cols[col][start: self._n] for col in _OHLCV_COLS},
                index=idx,
                )
...
class BacktestEngine:
    ...
    history_tail: int | None = None # if set, strategies can only see the last n bars
    ...
    def __post_init__(self) -> None:
        ...
        self._buffers = {symbol: _SymbolBuffer() for symbol in self.data}
        ...
        for i, ts in enumerate(all_index):
            ...
            for symbol in self.data:
                ...
                for fill in fills:
                    ...
                    self.portfolio.apply_fill(symbol,signed_qty,fill.fill_price, fill.commission)
                    trades.append({
            ...
            if i>= warmup and self._should_rebalance(ts,last_rebalance_ts):
                for symbol in self.data:
                    ...
                    if idx is None:
                        continue
                    cc = col_cache[symbol]
                    bar = pd.Series({col: cc[col][idx] for col in _OHLCV_COLS}, name = ts)
            ...
            snap = {symbol: position for symbol, position in self.portfolio.positions.items()}
            snap["_ts"] = ts
