...
def generate_gbm(n:int=252,s0:float=100.0,mu:float=0.08,sigma:float=0.2,dt:float=1/252,seed:int | None=None) -> pd.Series:
        """Generate a Geometric Brownian Motion price series."""
        range = np.random.default_rng(seed)
        z = range.standard_normal(n)
...
def generate_panel(
    ...
    """Generate a dict of OHLCV frames keyed by symbol (small panel)."""
    out = {}
