...
def historical_var(returns, confidence_level: float = 0.95, horizon: int = 1) -> float:
    """Returns a POSITIVE number representing the loss threshold."""
    r = _arr(returns)
    if len(r) == 0:
        return np.nan
    q = np.quantile(r, 1 - confidence_level)
    return float(-q * np.sqrt(horizon))

def historical_cvar(returns, confidence_level: float = 0.95, horizon: int = 1) -> float:
    r = _arr(returns)
    if len(r) == 0:
        return np.nan
    q = np.quantile(r, 1 - confidence_level)
